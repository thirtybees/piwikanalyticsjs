...
<button onclick=\"return submitPiwikSiteAPIUpdate()\"
        ...
        if (method_exists('PKHelper', $apiMethod) && isset(PKHelper::$acp[$apiMethod])) {
            ...
            foreach ($required as $requiredOption) {
                if (!Tools::getIsset($requiredOption)) {
                    ...
                    die(Tools::jsonEncode(['error' => true, 'message' => sprintf($this->l('Required parameter "%s" is missing'), $requiredOption)]));
                }
            }
            ...
            if ($result === FALSE) {
                ...
                if (!empty(PKHelper::$errors))
                    $lastError = "\n".PKHelper::$error;
                die(Tools::jsonEncode(['error' => TRUE, 'message' => sprintf($this->l('Unknown error occurred%s'), $lastError)]));
            } else {
                PKHelper::DebugLogger("__pkapicall():\n\t- Al good");
                ...
                if (is_bool($message)) {
                    die(Tools::jsonEncode(['error' => FALSE, 'message' => $this->l('Successfully Updated')]));
                } else {
                    die(Tools::jsonEncode(['error' => FALSE, 'message' => $message]));
                }
            }
        } else {
            die(Tools::jsonEncode(['error' => true, 'message' => sprintf($this->l('Method "%s" dos not exists in class PKHelper'), $apiMethod)]));
        }
    }
    ...
    private function processFormsUpdate()
    {
        ...
        if (Tools::isSubmit('submitUpdate'.$this->name)) {
            ...
            if (Tools::getIsset(PKHelper::CPREFIX.'PROXY_SCRIPT'))
                Configuration::updateValue(PKHelper::CPREFIX.'PROXY_SCRIPT', str_replace(["http://", "https://", '//'], '', Tools::getValue(PKHelper::CPREFIX.'PROXY_SCRIPT')));
    ...
    {
        $this->default_currency = ['value' => 0, 'label' => $this->l('Choose currency')];
    ...
    protected function getFormFields()
    {
        ...
        return [
            ...
            PKHelper::CPREFIX.'PROXY_SCRIPT'     => empty($PIWIK_PROXY_SCRIPT) ? str_replace(["http://", "https://"], '', self::getModuleLink($this->name, 'piwik')) : $PIWIK_PROXY_SCRIPT,
    ...
    private function _hookFooterPS14($params, $page_name)
    {
        ...
        if (strtolower($page_name) == "product" && isset($_GET['id_product']) && Validate::isUnsignedInt($_GET['id_product'])) {
            ...
            $this->context->smarty->assign([PKHelper::CPREFIX.'PRODUCTS' => $smarty_ad]);
            unset($smarty_ad);
        }
        /* category tracking */
        if (strtolower($page_name) == "category" && isset($_GET['id_category']) && Validate::isUnsignedInt($_GET['id_category'])) {
            ...
            $this->context->smarty->assign([
                PKHelper::CPREFIX.'category' => ['NAME' => $category->name],
            ]);
        }
    }
